...
- **Puertos Expuestos:** 
  - `80:80` (HTTP)
  - `443:443` (HTTPS)
- **Función:** Punto de entrada, proxy reverso, servidor estático
- **Volúmenes:**
  - `./nginx/nginx.conf` → `/etc/nginx/nginx.conf`
  - `./nginx/default.conf` → `/etc/nginx/conf.d/default.conf`
  - `./processed_videos` → `/var/www/processed_videos` (read-only)
...
- **Función:** Dashboard de monitoreo de Celery
- **Variables de Entorno:**
  - `CELERY_BROKER_URL=redis://redis:6379/0`
- **Comando:** 
  ```bash
...
3. **./logs** - Logs de aplicación
4. **./nginx/** - Configuración de Nginx

### Red Docker

**Nombre:** `anb_network`  
**Tipo:** `bridge`  
**Función:** Conecta todos los contenedores en una red privada

**Resolución DNS Interna:**
- `postgres` → IP del contenedor PostgreSQL
- `redis` → IP del contenedor Redis
- `api` → IP del contenedor FastAPI

### Puertos y Protocolos

| Puerto | Protocolo | Servicio | Acceso | Descripción |
|--------|-----------|----------|--------|-------------|
| 80 | HTTP | Nginx | Público | API REST y archivos estáticos |
| 443 | HTTPS | Nginx | Público | API REST segura (preparado) |
...
| 5555 | HTTP | Flower | Público | Dashboard de Celery |

**Aclaraciones:**
- **HTTP/HTTPS**: Protocolo de aplicación sobre TCP/IP
...
- **File I/O**: Acceso a volúmenes Docker via filesystem del host (no red)
...
- Descarga imágenes: postgres:15-alpine, redis:7-alpine, nginx:alpine
...
- Solo Nginx y Flower expuestos públicamente
- PostgreSQL y Redis NO accesibles desde fuera

#### Nivel de Aplicación
- Variables de entorno para credenciales
- Secrets recomendados para producción
...
#### Horizontal
- **API:** Múltiples replicas detrás de Nginx
...
**Configuración:** Variables de entorno en `.env` o `docker-compose.yml`  
